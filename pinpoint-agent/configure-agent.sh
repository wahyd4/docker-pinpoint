...
PINPOINT_AGENT_MAPPED_CONFIG_FILE=${PINPOINT_AGENT_MAPPED_CONFIG_FILE:-/assets/pinpoint-agent/pinpoint.config}

if [ -f ${PINPOINT_AGENT_MAPPED_CONFIG_FILE} ]; then
	cp ${PINPOINT_AGENT_MAPPED_CONFIG_FILE} ${PINPOINT_AGENT_HOME} 
fi
...
PROFILER_INCLUDE=${PROFILER_INCLUDE}
PROFILER_ENTRYPOINT=${PROFILER_ENTRYPOINT}
...
if [ "$PROFILER_INCLUDE" != "" ]; then
    sed -i "/profiler.include=/ s/=.*/=${PROFILER_INCLUDE}/" ${PINPOINT_AGENT_HOME}/pinpoint.config
fi
if [ "$PROFILER_ENTRYPOINT" != "" ]; then
    sed -i "/profiler.entrypoint=/ s/=.*/=${PROFILER_ENTRYPOINT}/" ${PINPOINT_AGENT_HOME}/pinpoint.config
fi
